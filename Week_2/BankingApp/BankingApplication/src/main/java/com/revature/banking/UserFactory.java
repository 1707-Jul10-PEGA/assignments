...
public class UserFactory {
	...
	public User createUserWithConsoleInput(int customerIndex) {
		...
		int U_ID = Integer.parseInt(input.nextLine());
			if("customer".equals(type)) {
				user = new Customer(firstName, lastName, age, phone, address, username,
						password, new ArrayList<Integer>(), customerIndex, U_ID);
				Main.customerBA.add(user);
				Random r = new Random();
				...
				*/
				...
				user = new Employee(firstName, lastName, age, phone, address, username,password,new ArrayList<Integer>(),U_ID);
				Main.employeeBA.add(user);
			}
			else if("admin".equals(type)) {
				user = new Administrator(firstName, lastName, age, phone, address, username,password,U_ID);
				Main.adminBA.add(user);
			}
