...
public class BalanceViewer {
	 ...
	 */
	public static String viewBalance(String user) throws IOException, SQLException
	{
		...
		Storage.userID = valueBalance.getLoginID();
		Storage.firstName = valueBalance.getFirstName();
		Storage.lastName = valueBalance.getLastName();
		Storage.money = valueBalance.getBalance();
		return Storage.print();
		
		/*
		 * Old method when Banking app used to use txt files to persist
		 */
...
//		return "No account balance associated with this user\n";
	 ...
	 */
	public static void approveOrDeny() throws IOException, SQLException
	{
		Scanner scanLine = new Scanner(System.in);
		
		BankingAppDaoImpl connect = new BankingAppDaoImpl();
		BalanceInfo valueBalance = new BalanceInfo();
		valueBalance = connect.getBalanceInfo("filler", 2);
		if(valueBalance == null)
		{
		...
		/*
		 * Old method used in previous banking app that persisted using txt files
		 */
